...
class GPTFinetuneDataset(MegatronDataset):
    ...
    def __init__(
        ...
        print("dataset length",self.num_samples)
        
        # print("document",self.document_index)
        # print("sample",self.sample_index)
        # print("shuffle",self.shuffle_index)
    ...
    def __getitem__(self, idx: Optional[int]) -> Dict[str, torch.Tensor]:
        ...
        if idx is None:
            ...
            text, _, instruction_len = self._query_document_sample_shuffle_indices(0)
        else:
            text, _, instruction_len = self._query_document_sample_shuffle_indices(idx)
            
        instruction_len = min(instruction_len, self.config.sequence_length)
        ...
        # print(tokens.size(), labels.size(), loss_mask.size(), position_ids.size())
        loss_mask[labels == self._pad_token_id] = 0.0
        for i in range(instruction_len-1):
            loss_mask[i] = 0.0
        if instruction_len >= self.config.sequence_length:
            loss_mask[-1] = 1.0
            
        # print(instruction_len)
        # print(tokens)
        # print(labels[instruction_len-1:])
        # print(loss_mask[instruction_len-1:], sum(loss_mask))
        # print(position_ids)
    ...
    def _query_document_sample_shuffle_indices(
        ...
        doc_idx = self.document_index[idx]*2 % len(self.dataset)
        sample_parts.append(self.dataset[doc_idx])
        sample_parts.append(self.dataset[doc_idx+1])
        document_ids.append(self.document_index[idx]*2)
        document_ids.append(self.document_index[idx]*2+1)
        ...
        output = numpy.concatenate(sample_parts, dtype=numpy.int64)
        if output.size > self.config.sequence_length + self.config.add_extra_token_to_sequence:
            output = output[: self.config.sequence_length + self.config.add_extra_token_to_sequence]
        return (
            output,
            numpy.array(document_ids, dtype=numpy.int64),
            sample_parts[0].__len__() if sample_parts[0].__len__() < self.config.sequence_length else self.config.sequence_length,
        )
...
class MockGPTDataset(GPTFinetuneDataset):
