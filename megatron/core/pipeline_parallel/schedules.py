...
from megatron.my_stage_distribution import my_p2p_communication
# Types
...
def get_forward_backward_func():
    ...
    if pipeline_model_parallel_size > 1:
        if parallel_state.get_virtual_pipeline_model_parallel_world_size() is not None:
            ...
            # elif parallel_state.get_virtual_pipeline_model_parallel_world_size() == 1:
            #     forward_backward_func = forward_backward_pipelining_overlap_communication
            else:
...
def forward_backward_pipelining_with_interleaving(
    *,
    ...
    combine_tensor_flag = config.overlap_p2p_comm 
    ...
    def forward_step_helper(microbatch_id, current_microbatch, checkpoint_activations_microbatch):
        ...
        if combine_tensor_flag and input_tensor is not None:
            with torch.no_grad():
                input_tensor = torch.cat(input_tensor, dim=1) if len(input_tensor) > 1 else input_tensor[0]
            input_tensor.requires_grad = True
            input_tensors[model_chunk_id][-1] = input_tensor
    ...
    def backward_step_helper(microbatch_id):
        ...
        #     with torch.no_grad():
        #         input_tensor = torch.cat(input_tensor, dim=1) if len(input_tensor) > 1 else input_tensor[0]
        if combine_tensor_flag and output_tensor_grad is not None:
            with torch.no_grad():
                output_tensor_grad = torch.cat(output_tensor_grad, dim=1) if len(output_tensor_grad) > 1 else output_tensor_grad[0]
    ...
    # def overlap_recv_forward(tensor_shape, config):
    #     """Receive the forward tensor from the previous pipeline stage."""
    #     if parallel_state.is_pipeline_first_stage(ignore_virtual=True):
    #         return None
    #     input_tensor = p2p_communication.recv_forward(tensor_shape, config)
    #     return input_tensor
    # print("overlap p2p comm", config.overlap_p2p_comm)
    # Run warmup forward passes.
    parallel_state.set_virtual_pipeline_model_parallel_rank(0)
    input_tensors[0].append(p2p_communication.recv_forward(tensor_shape, config) if not combine_tensor_flag else my_p2p_communication.recv_forward(tensor_shape, config))
    ...
    for k in range(num_warmup_microbatches):
        ...
        if not config.overlap_p2p_comm:
            ...
            input_tensor, fwd_wait_handles = my_p2p_communication.send_forward_recv_forward(
                output_tensor,
                recv_prev=recv_prev,
                tensor_shape=tensor_shape,
                config=config,
                overlap_p2p_comm=True,
            ) if combine_tensor_flag else \
            ...
            if (
                ...
                (
                    output_tensor_grad,
                    bwd_wait_handles,
                ) = my_p2p_communication.send_backward_recv_backward(
                    input_tensor_grad,
                    recv_next=recv_next,
                    tensor_shape=tensor_shape,
                    config=config,
    ...
    for k in range(num_microbatches_remaining):
        ...
        if config.overlap_p2p_comm:
            ...
            output_tensor_grad, bwd_wait_handles = my_p2p_communication.send_backward_recv_backward(
                input_tensor_grad,
                recv_next=recv_next,
                tensor_shape=tensor_shape,
                config=config,
    ...
    if not forward_only:
        ...
        if all_warmup_microbatches:
            output_tensor_grads[num_model_chunks - 1].append(
                my_p2p_communication.recv_backward(tensor_shape, config=config) if combine_tensor_flag else p2p_communication.recv_backward(tensor_shape, config=config)
            )
        for k in range(num_microbatches_remaining, total_num_microbatches):
            ...
            output_tensor_grads[next_backward_model_chunk_id].append(
                ...
                ) if combine_tensor_flag else p2p_communication.send_backward_recv_backward(
                    input_tensor_grad, recv_next=recv_next, tensor_shape=tensor_shape, config=config
                )
            )
...
def recv_forward(tensor_shapes, config, overlap_p2p_comm=False):
    input_tensors = []
    for tensor_shape in tensor_shapes:
        if tensor_shape is None:
            input_tensors.append(None)
        else:
            if config.non_uniform_dp_per_stage:
                input_tensors.append(my_p2p_communication.recv_forward(tensor_shape, config))
            else:
                input_tensors.append(p2p_communication.recv_forward(tensor_shape, config, overlap_p2p_comm))
    return input_tensors


def recv_backward(tensor_shapes, config, overlap_p2p_comm=False):
    output_tensor_grads = []
    for tensor_shape in tensor_shapes:
        if tensor_shape is None:
            ...
            if config.non_uniform_dp_per_stage:
                output_tensor_grads.append(my_p2p_communication.recv_backward(tensor_shape, config))
            else:
                output_tensor_grads.append(p2p_communication.recv_backward(tensor_shape, config, overlap_p2p_comm))
    return output_tensor_grads


def send_forward(output_tensors, tensor_shapes, config, overlap_p2p_comm=False):
    ...
    for (output_tensor, tensor_shape) in zip(output_tensors, tensor_shapes):
        if tensor_shape is None:
            continue
        if config.non_uniform_dp_per_stage:
            my_p2p_communication.send_forward(output_tensor, config)
        else:
            p2p_communication.send_forward(output_tensor, config, overlap_p2p_comm)


def send_backward(input_tensor_grads, tensor_shapes, config, overlap_p2p_comm=False):
    ...
    for (input_tensor_grad, tensor_shape) in zip(input_tensor_grads, tensor_shapes):
        if tensor_shape is None:
            continue
        if config.non_uniform_dp_per_stage:
            my_p2p_communication.send_backward(input_tensor_grad, config)
        else:
            p2p_communication.send_backward(input_tensor_grad, config, overlap_p2p_comm)


def send_forward_recv_backward(output_tensors, tensor_shapes, config, overlap_p2p_comm=False):
    if not isinstance(output_tensors, list):
        output_tensors = [output_tensors]
    output_tensor_grads = []
    for (output_tensor, tensor_shape) in zip(output_tensors, tensor_shapes):
        if tensor_shape is None:
            ...
            continue
        if config.non_uniform_dp_per_stage:
            output_tensor_grad = my_p2p_communication.send_forward_recv_backward(
...
def send_backward_recv_forward(input_tensor_grads, tensor_shapes, config, overlap_p2p_comm=False):
    if not isinstance(input_tensor_grads, list):
        input_tensor_grads = [input_tensor_grads]
    input_tensors = []
    for (input_tensor_grad, tensor_shape) in zip(input_tensor_grads, tensor_shapes):
        if tensor_shape is None:
            ...
            continue
        if config.non_uniform_dp_per_stage:
            input_tensor = my_p2p_communication.send_backward_recv_forward(
...
def forward_backward_pipelining_overlap_communication(
    *,
    forward_step_func,
    data_iterator: Union[Iterator, List[Iterator]],
    model: Union[torch.nn.Module, List[torch.nn.Module]],
    ...
    if config.overlap_p2p_comm and config.batch_p2p_comm:
        raise ValueError("Can not use both overlap_p2p_comm and batch_p2p_comm")
    assert config.overlap_p2p_comm, "forward_backward_pipelining_overlap_communication expected overlap_p2p_comm"
    ...
    for k in range(num_warmup_microbatches):
        ...
        input_tensor, fwd_wait_handles = p2p_communication.recv_forward(
            tensor_shape=tensor_shape,
            config=config,
            overlap_p2p_comm=True,
        )
