...
def clone(func, *args, **kwargs):
    ...
    metadata = SubclassTensorArgs(
            args[0][0].size(),
            args[0][0].stride(),
            args[0][0].storage_offset(),
            args[0][0].dtype,
            args[0][0].device,
            args[0][0].requires_grad,
        )
    new_tensor = NF4Tensor(
        metadata, 
        args[0][0].block_size, 
        args[0][0].n_blocks, 
        args[0][0].scaler_block_size, 
        args[0][0].quantized_scalers.clone().detach(), 
        args[0][0].quantization_factor.clone().detach(), 
        args[0][0].scaler_mean.clone().detach(), 
        args[0][0].quantized_data.clone().detach(), 
        args[0][0].nf4.clone().detach()
    )
    return new_tensor
